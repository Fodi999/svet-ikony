...
export async function POST(request: Request) {
  return withErrors(async () => {
    ...
    if (typeof mediaModule !== 'string' || !mediaModule) throw ApiError.validation('module is required');
    ...
    if (!isAllowedModule(mediaModule)) throw ApiError.validation(`Unknown module: ${mediaModule}`);
    if (!isSafeEntityId(entityId)) throw ApiError.validation('entityId contains unsafe characters');
    if (!isAllowedPurpose(mediaModule, purpose)) throw ApiError.validation(`Purpose "${purpose}" is not allowed for module "${mediaModule}"`);

    if (file.size === 0) throw ApiError.validation('file is empty');

    const kind = mediaKindForPurpose(purpose);
    const contentType = kind === 'model' && (!file.type || file.type === 'application/octet-stream') ? 'model/gltf-binary' : file.type;
    const allowedMimeForKind = kind === 'model' ? MODEL_MIME_EXTENSIONS : kind === 'audio' ? AUDIO_MIME_EXTENSIONS : IMAGE_MIME_EXTENSIONS;
    if (!Object.prototype.hasOwnProperty.call(allowedMimeForKind, contentType)) {
    ...
    const key = generateMediaKey({ module: mediaModule, entityId, purpose, mimeType: contentType });
    ...
    const putResult = await bucket.put(key, body, {
      ...
      customMetadata: { module: mediaModule, entityId, purpose, filename: file.name.replace(/[\\/]/g, '_').slice(0, 200) },
    });
