import { assignSlotImage, removeSlotImage } from '@/lib/telegram/content-plan-actions';
...
/** "Видалити фото" -- clears the slot's media_url. See
 * content-plan-actions.ts's removeSlotImage(). */
export async function DELETE(request: Request, { params }: { params: Promise<{ date: string; type: string }> }) {
  return handleSlotAction(request, params, removeSlotImage);
}
