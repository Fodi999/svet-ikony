...
const mockMarkSlotReady = vi.fn();
vi.mock('@/lib/telegram/content-plan-actions', () => ({ markSlotReady: mockMarkSlotReady }));
...
function req(token?: string, body?: unknown) {
  return new Request('http://localhost/api/admin/telegram/content-plan/2026-09-02/morning_prayer/ready', {
    method: 'POST',
    headers: {
...
describe('POST .../content-plan/:date/:type/ready', () => {
  let token: string;
  beforeEach(async () => {
    vi.clearAllMocks();
  ...
  it('rejects an unauthenticated request with 401 without calling the action', async () => {
    ...
    expect(mockMarkSlotReady).not.toHaveBeenCalled();
  });
  ...
  it('calls markSlotReady with the date/type and returns its result', async () => {
    mockMarkSlotReady.mockResolvedValue({ id: 1, status: 'draft' });
    const response = await POST(req(token, undefined), ctx('2026-09-02', 'morning_prayer'));
    expect(response.status).toBe(200);
    expect(mockMarkSlotReady).toHaveBeenCalledWith('2026-09-02', 'morning_prayer');
  ...
  it('propagates an error from the action as an error response, never a 200', async () => {
    ...
    mockMarkSlotReady.mockRejectedValue(ApiError.conflict('already sent'));
    const response = await POST(req(token, undefined), ctx('2026-09-02', 'morning_prayer'));
