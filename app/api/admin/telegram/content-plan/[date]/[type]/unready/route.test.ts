...
const mockMarkSlotUnready = vi.fn();
vi.mock('@/lib/telegram/content-plan-actions', () => ({ markSlotUnready: mockMarkSlotUnready }));
...
function req(token?: string, body?: unknown) {
  return new Request('http://localhost/api/admin/telegram/content-plan/2026-09-02/morning_prayer/unready', {
    method: 'POST',
    headers: {
...
describe('POST .../content-plan/:date/:type/unready', () => {
  let token: string;
  beforeEach(async () => {
    vi.clearAllMocks();
  ...
  it('rejects an unauthenticated request with 401 without calling the action', async () => {
    ...
    expect(mockMarkSlotUnready).not.toHaveBeenCalled();
  });
  ...
  it('calls markSlotUnready with the date/type and returns its result', async () => {
    mockMarkSlotUnready.mockResolvedValue({ id: 1, status: 'draft' });
    const response = await POST(req(token, undefined), ctx('2026-09-02', 'morning_prayer'));
    expect(response.status).toBe(200);
    expect(mockMarkSlotUnready).toHaveBeenCalledWith('2026-09-02', 'morning_prayer');
  ...
  it('propagates an error from the action as an error response, never a 200', async () => {
    ...
    mockMarkSlotUnready.mockRejectedValue(ApiError.conflict('already sent'));
    const response = await POST(req(token, undefined), ctx('2026-09-02', 'morning_prayer'));
