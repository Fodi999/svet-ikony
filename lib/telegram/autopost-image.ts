 ...
 * elsewhere] -> here (OpenAI image -> R2 -> media_url) -> Telegram
 * sendPhoto+caption. Never blocks the text-only publish path -- every
 * failure (OpenAI or R2) is caught, recorded via setAutopostImageResult's
 * `imageError`, and reported back as `null` so callers fall back to
 * sendMessage. See migration 0009_telegram_autopost_images.sql. */
