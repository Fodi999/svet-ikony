 ...
 * sendPhoto's own caption; one that doesn't goes out as a photo-only
 * message immediately followed by a separate full-text message. Both
...
export async function sendAutopostMessage(input: SendAutopostMessageInput): Promise<SendAutopostMessageResult> {
  ...
  if (!photoMessageId) {
    const photoResult = await input.client.sendPhoto(input.chatId, input.mediaUrl as string);
